...
function display() {
  ...
  http.open("GET", "/admin/getListQuestions", true);
...
function displayQuestion(question) {
  ...
  var buttonPublic =
    '<div class="form-check form-switch pt-2 me-4">' +
    '<input class="form-check-input" type="checkbox" onClick="publicQuestion(\'' +
    question.ID +
  ...
  let button =
    '<button class="btn btn-success add-answer" onClick="addAnswer(\'' + question.ID + '\')"><span class="fa fa-plus"></span> Thêm lựa chọn</button>';
...
function removeQuestion(id) {
  ...
  var http = new XMLHttpRequest();
  http.open("POST", "/admin/deleteQuestion", true);
  http.setRequestHeader("Content-type", "application/json");
...
function addQuestion() {
  ...
  var http = new XMLHttpRequest();
  http.open("POST", "/admin/addQuestion", true);
  http.setRequestHeader("Content-type", "application/json");
...
function removeAnswer(answerID, questionID) {
  ...
  console.log(params);
  var http = new XMLHttpRequest();
  http.open("POST", "/admin/deleteAnswer", true);
  http.setRequestHeader("Content-type", "application/json");
...
function addAnswer(idQuestion) {
  ...
  var http = new XMLHttpRequest();
  http.open("POST", "/admin/addAnswer", true);
  http.setRequestHeader("Content-type", "application/json");
...
function publicQuestion(id) {
  ...
  var http = new XMLHttpRequest();
  http.open("POST", "/admin/publicQuestion", true);
  http.setRequestHeader("Content-type", "application/json");
...
/*function changePassword(){
    ...
    var http = new XMLHttpRequest();
    http.open("POST", "/admin/changePassword", true);
    http.setRequestHeader("Content-type","application/json");
    http.send(JSON.stringify(params));
    http.onload = function() {
        if(http.responseText=="success"){
